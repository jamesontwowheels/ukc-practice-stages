    ...
    const widgetID_3 = 3; // Replace with dynamic widget_ID if needed
    console.log('ping 3');
    // Function to fetch data and process it for the count per day
    function fetchWidgetData3() {
        fetch(`assets/php/get_data.php?purpose=${widgetID_3}`) // Adjust API endpoint as needed
            .then(response => response.json())
            .then(data => {
                // Filter data where Purpose matches widgetID
                const filteredData = data.filter(item => item.Purpose == widgetID_3);
                ...
                createBarChart3(lastSevenDays, Object.values(dailyCounts), totalLast7Days, overallTotal);
            })
            .catch(error => console.error("Error fetching data:", error));
    }

    // Function to create a bar chart showing daily input counts
    function createBarChart3(labels, data, totalLast7Days, overallTotal) {
        // Create a container for the chart
        const widgetZone = document.getElementById("widget_zone_3");
        if (!widgetZone) {
            console.error("Element with ID 'widget_zone_3' not found.");
            return;
        }
